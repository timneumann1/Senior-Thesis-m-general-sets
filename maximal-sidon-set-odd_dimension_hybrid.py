...
and another (x,x^2) construction in (even) dimension (n-1)/2. 
@author: Tim Neumann
"""
...
def main():
    ...
    if (n % 4 != 0):
        print("Please enter odd integer n such that n = 4k+1 for some non-negative integer k.")
        exit()
    ...
    # adding points (0,x,x^2) for x in AG( (n-1)/2, 3)
    ...
    # adding points from additional (x,x^2) construction to add maximal Sidon set in dimension (n-1) / 4        
    GF = galois.GF(3**int(n / 4), repr="int")
    print(GF.properties)
    print(GF.repr_table())
    ...
    for i in range(GF.order):
        ...
        # Concatenate to form (1,0,y,y^2), where y = (y,y^2) \in \F_3^{n/2} is a Sidon set
        
        num_zeros = n // 2
        sidon_element = np.concatenate(([1],np.array(square_element_vec)[::-1],np.array(element_vec)[::-1],np.zeros(num_zeros, dtype = int)))    
        sidon_set.append(sidon_element)
